...
const saveUserInfo = async () => {
  ...
  if (editableBirthday.value > today) {
    await Swal.fire("錯誤", "生日不能超过今天的日期", "error");
    return; // 提前终止函数执行
  }
...
.cancel-btn {
  background-color: #d9534f; /* 取消按钮的背景色 */
}

.cancel-btn:hover {
  background-color: #c9302c; /* 取消按钮的背景色 */
}

.edit-section {
  display: flex;
  flex-direction: column;
  align-items: flex-start; /* 确保编辑区域的内容靠左对齐 */
}

.edit-label,
.edit-input {
  text-align: left; /* 标签和输入框内容靠左对齐 */
  margin-bottom: 10px; /* 为标签和输入框添加一些间距 */
}

.edit-input {
  padding: 10px;
  margin: 5px 0; /* 调整输入框的上下外边距 */
  border: 1px solid #ccc;
  border-radius: 4px;
}
</style>
